...
static int zcrx_validate_notif(struct io_zcrx_ifq *ifq,
			  ...
			  struct io_uring_zcrx_ifq_reg __user *arg)
{
	...
	if (notif.flags & ZCRX_NOTIF_DESC_FLAG_STATS) {
		ret = zcrx_validate_notif(ifq, &reg, &notif);
		if (ret)
			goto err;
	}
...
static void zcrx_stat_inc(__u64 *p)
{
	atomic64_inc((atomic64_t *)p);
}

static void zcrx_stat_add(s64 v, __u64 *p)
{
	atomic64_add(v, (atomic64_t *)p);
}
			     ...
			     const skb_frag_t *frag, int off, int len)
{
	...
	if (ret > 0) {
		if (ifq->notif_stats) {
			zcrx_stat_inc(&ifq->notif_stats->copy_count);
			zcrx_stat_add(ret, &ifq->notif_stats->copy_bytes);
		}
