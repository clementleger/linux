...
static int of_bar_remap(struct pci_dev *pci_dev, int bar, u64 offset, int val[7])
{
	...
	/* Bus address */
	val[0] = cpu_to_be32(offset);

	/* PCI bus address */
	val[1] = cpu_to_be32(0x2 << 24);
	val[2] = cpu_to_be32(start >> 32);
	val[3] = cpu_to_be32(start & 0xFFFFFFFF);

	/* Size */
	size = pci_resource_len(pci_dev, bar);
	val[4] = cpu_to_be32(size & 0xFFFFFFFF);
...
{
	struct device_node *itc_node, *node;
	struct device *dev = &pdev->dev;
	struct lan966x_pci *data;
	int ret;

	node = pci_get_of_node(pdev);
	if (!node) {
