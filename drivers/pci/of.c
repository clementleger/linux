...
static struct device_node *of_pci_create_bus_node(struct pci_bus *bus)
{
	...
	if (pci_is_root_bus(bus)) {
		bus_node = of_pci_create_root_bus_node(bus);
		if (!bus_node)
			return NULL;

		device_set_node(&bus->dev, of_fwnode_handle(bus_node));
...
static struct device_node *of_pci_make_dev_node(struct pci_dev *dev)
{
	...
	parent = of_pci_create_bus_node(dev->bus);
	if (!parent)
		return NULL;
