...
static void a5psw_vlan_set_tagged(struct a5psw *a5psw, int vlan_res_id,
				  int port, bool set)
{
...
static void a5psw_vlan_port_add(struct a5psw *a5psw, int vlan_res_id, int port,
				bool tagged, bool set)
{
	...
	a5psw_vlan_set_tagged(a5psw, vlan_res_id, port, tagged);
}
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	dev_dbg(a5psw->dev, "Add VLAN %d on port %d, %s, %s\n",
		vlan->vid, port, tagged ? "tagged" : "untagged",
		pvid ? "PVID" : "no PVID");
	...
	a5psw_vlan_port_add(a5psw, vlan_res_id, port, tagged, true);
				 ...
				 const struct switchdev_obj_port_vlan *vlan)
{
	...
	a5psw_vlan_port_add(a5psw, vlan_res_id, port, false, false);
